...
namespace SorrowServer
{
    ...
    public class SorrowServer : TerrariaPlugin
    {
        ...
        public override void Initialize()
        {
            ...
            ServerApi.Hooks.ItemForceIntoChest.Register(this, ItemForceIntoChest);
        ...
        private HookResult NpcPreUpdate(NPC npc, ref int i)
        {
                ...
                switch (args.Parameters[0])
                {
                    case "allow":
                    {
                        ...
                        if (int.TryParse(args.Parameters[1], out id))
                        {
                            ...
                            {
                                args.Player.SendErrorMessage($"Lista nie zawiera {id}" +
                                                             (id < 0
                                                                 ? "."
                                                                 : $" czyli {TShock.Utils.GetNPCById(id).FullName}."));
                                return;
                            }

                            config.BlockedNPCs.Remove(id);
                            config.Write("config.json");
                            args.Player.SendSuccessMessage($"Pomyslnie zezwolono {id}" +
                                                           (id < 0
                                                               ? "."
                                                               : $" czyli {TShock.Utils.GetNPCById(id).FullName}."));
                        ...
                        {
                            args.Player.SendErrorMessage($"Lista nie zawiera {npcs[0].type}" +
                                                         (id < 0 ? "." : $" czyli {npcs[0].FullName}."));
                            return;
                        }

                        config.BlockedNPCs.Remove(npcs[0].type);
                        config.Write("config.json");
                        args.Player.SendSuccessMessage($"Pomyslnie zezwolono {npcs[0].type}" +
                                                       (id < 0 ? "." : $" czyli {npcs[0].FullName}."));
                        return;
                    }
                    case "deny":
                    {
                        ...
                        if (int.TryParse(args.Parameters[1], out id))
                        {
                            if (config.BlockedNPCs.Contains(id))
                            {
                                args.Player.SendErrorMessage($"Lista juz zawiera {id}" +
                                                             (id < 0
                                                                 ? "."
                                                                 : $" czyli {TShock.Utils.GetNPCById(id).FullName}."));
                                return;
                            ...
                            config.Write("config.json");
                            args.Player.SendSuccessMessage($"Pomyslnie zabroniono {id}" +
                                                           (id < 0
                                                               ? "."
                                                               : $" czyli {TShock.Utils.GetNPCById(id).FullName}."));
                        ...
                        {
                            args.Player.SendErrorMessage($"Lista juz zawiera {id}" +
                                                         (id < 0 ? "." : $" czyli {NPC.getNewNPCName(id)}."));
                            return;
                        }

                        config.BlockedNPCs.Add(npcs[0].type);
                        config.Write("config.json");
                        args.Player.SendSuccessMessage($"Pomyslnie zabroniono {npcs[0].type}" +
                                                       (id < 0 ? "." : $" czyli {npcs[0].FullName}."));
